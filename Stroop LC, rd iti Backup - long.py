...
def send_keepalive(outlet):
    ...
    while True:
        try:
            ...
            print(f"⚠️ Keepalive failed: {str(e)}")
        time.sleep(5.0)
...
dlg = gui.DlgFromDict(dictionary=exp_info, title='Stroop Task - LOW CONTRAST')
if not dlg.OK:
    core.quit()  # Cancel was pressed

# Set up the experiment window with neutral gray background
...
# Define color RGB values for text
color_values = {
    "red": [1.0, 0.0, 0.0],     # Desaturated red
    "green": [0.0, 1.0, 0.0],   # Desaturated green
    "blue": [0.0, 0.0, 1.0],    # Desaturated blue
    "yellow": [1.0, 1.0, 0.0],  # Desaturated yellow
...
raw_data_file = f"stroop_low_contrast_raw_{exp_info['participant']}_{exp_info['session']}.csv"
summary_data_file = f"stroop_low_contrast_summary_{exp_info['participant']}_{exp_info['session']}.csv"
results = []
...
def create_stroop_stimulus(stimulus_code):
    for color_name in ["red", "green", "blue", "yellow"]:
        if stimulus_code.startswith(color_name):
            ...
            if color == "":  # <-- Add this block
                color = word  # For congruent trials
            break
    
    # Create text with desaturated color
    text = visual.TextStim(
        win, 
        text=stroop_text[word], 
        color=color_values[color],
        opacity=0.1,  # Adjust this value (0.1-0.3) to control visibility
        height=80, 
        bold=True, 
        pos=[0, 0]
    )
...
def run_congruent_block(block_num, num_trials=8):
    ...
    for trial in block_trials:
        ...
        send_event_code(outlet, 1, f'trial_start_{trial["stimulus"]}')  # Code 1 for all trial starts
        bg, stim = create_stroop_stimulus(trial["stimulus"])
        bg.draw()
        stim.draw()
        win.flip()
...
block_sequence = [
    ("congruent", 1),           # Default: 8 trials
    ("neutral", None),
    ("incongruent", 2),         # Default: 8 trials
    ("neutral", None),
    ("congruent", 3, 15),       # Block 3: 15 trials
    ("neutral", None),
    ("incongruent", 4, 15),     # Block 4: 15 trials
    ("neutral", None),
    ("congruent", 5),           # Default: 8 trials
    ("neutral", None),
    ("incongruent", 6),         # Default: 8 trials
    ("neutral", None),
    ("congruent", 7, 15),       # Block 7: 15 trials
    ("neutral", None),
    ("incongruent", 8, 15)      # Block 8: 15 trials
]
...
for block_info in block_sequence:
    block_type = block_info[0]  # "congruent", "incongruent", or "neutral"
    block_num = block_info[1]   # Block number (1, 2, 3...)
    
    # Check if num_trials is specified (default to 8 if not)
    num_trials = block_info[2] if len(block_info) > 2 else 8

    if block_type == "congruent":
        # Send LSL marker for block start (e.g., CODE_101 for block 1)
        send_event_code(outlet, 100 + block_num, f'block_{block_num}_congruent_start')
        
        # Run the block with specified num_trials (15 for blocks 3,7; 8 otherwise)
        block_results = run_congruent_block(block_num, num_trials=num_trials)
        
        # Save results and send block-end marker
        results.extend(block_results)
        send_event_code(outlet, 150 + block_num, f'block_{block_num}_congruent_end')

    elif block_type == "incongruent":
        # Send LSL marker for block start (e.g., CODE_201 for block 2)
        send_event_code(outlet, 200 + block_num, f'block_{block_num}_incongruent_start')
        
        # Run the block with specified num_trials (15 for blocks 4,8; 8 otherwise)
        block_results = run_incongruent_block(block_num, num_trials=num_trials)
        ...
        send_event_code(outlet, 350, 'neutral_block_end')
        results.append({
            "participant": exp_info['participant'],
            "session": exp_info['session'],
            "block": "neutral",
